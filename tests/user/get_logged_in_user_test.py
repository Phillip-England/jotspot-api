...
def get_logged_in_user_test(session, print_results=True):
    ...
    def make_current_session_expire():
        ...
        update = {"$set": {'expires_at': datetime.datetime.now() -
    ...
    test_unit.build_test(
        ...
        status_code_expected=401,
        pre_execution_callback=test_unit.inject_function(
            make_current_session_expire, True, "to make current session expire"),
        post_execution_callback=test_unit.inject_function(
            attempt_to_get_deleted_session, True, 'to check if current session has been deleted')
    )
    ...
    test_unit = TestUnit(
        ...
        method='POST',
        path='/user/login',
        description="logging in the user again to restore deleted session",
        print_results=True
    )
